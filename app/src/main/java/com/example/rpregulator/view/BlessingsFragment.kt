...
class BlessingsFragment: Fragment() {
    ...
    override fun onCreateView(
        ...
        blessingsViewModel.navigateToAddBlessings.observe(viewLifecycleOwner, Observer {
            it?.let{
                val action = StatusFragmentDirections.actionStatusFragmentToAddBlessingFragment()
                findNavController().navigate(action)
            }
        })
